                ...
                for member in details["members"]:
                    if member["name"].lower() == member_target_name.lower():
                        results.append({
                            "name": member["name"],
                            "kind": member["kind"],
                            "refid": member.get("refid") or member.get("location", {}).get("file", "") + ":" + str(member.get("location", {}).get("line", "")),
            ...
            if parent_details and "members" in parent_details:
                for m in parent_details["members"]:
                    if m.get("refid") == refid:
                        if parent_details.get("name") and parent_details.get("kind") != "file":
    ...
    def get_virtual_diff(self, project_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        for rel_file in modified_files:
            ...
            for source_name, source_info in disk_sigs.items():
                xml_sym = find_matching_xml_symbol(source_name, source_info["kind"], xml_symbols)
                if xml_sym:
            ...
            for xml_sym in xml_symbols:
                ...
                if ref_key not in matched_xml_refids:
                    if xml_sym["kind"] in {"class", "struct", "function", "slot", "signal"}:
                        old_sig = f"{xml_sym['name']}{xml_sym.get('args') or ''}"
...
def parse_cpp_signatures_from_source(content: str) -> Dict[str, Dict[str, Any]]:
    ...
    for match in func_pattern.finditer(content_clean):
        ...
        if name in {"if", "for", "while", "switch", "catch", "return", "using", "template"}:
            continue
...
def find_matching_xml_symbol(source_name: str, source_kind: str, xml_symbols: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    for xml_sym in xml_symbols:
        ...
        if source_kind == "class" and xml_sym["kind"] not in {"class", "struct"}:
            continue
        if source_kind == "function" and xml_sym["kind"] not in {"function", "slot", "signal"}:
            continue
