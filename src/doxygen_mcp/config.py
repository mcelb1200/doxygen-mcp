...
class DoxygenConfig(BaseModel):
    ...
    def from_env(cls, **kwargs) -> "DoxygenConfig":
        ...
        from .utils import resolve_project_path, get_project_name  # pylint: disable=import-outside-toplevel
        ...
        # Fallback to __annotations__ if Pydantic metadata is missing
        # (e.g. in mocked environments)
    ...
    def to_doxyfile(self) -> str:
        ...
        if self.exclude_patterns:
            lines.append(
                f"EXCLUDE_PATTERNS       = {' '.join(self._sanitize_list(self.exclude_patterns))}"
            )
