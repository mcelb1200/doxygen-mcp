...
async def scan_project(
    ...
    # Handle asyncio.to_thread for Python < 3.9
    ...
    if hasattr(asyncio, "to_thread"):
        ...
        with concurrent.futures.ThreadPoolExecutor() as pool:
            extensions, total_files = await loop.run_in_executor(pool, _perform_scan, safe_project_path)
...
async def check_doxygen_install() -> str:
    ...
    try:
        ...
        d_version = stdout.decode(errors='replace').strip()
        return f"✅ Doxygen {d_version} is installed and working"
...
def main():
    ...
    d_exe = os.environ.get("DOXYGEN_PATH", "doxygen")
    try:
        subprocess.run([d_exe, "--version"], capture_output=True, check=True)
        ...
        # src/doxygen_mcp/server.py -> src/doxygen_mcp -> src -> root
        s_path = Path(__file__).parent.parent.parent / "scripts" / "check_environment.py"
        if s_path.exists():
            try:
                subprocess.run([sys.executable, str(s_path), "--install"], check=True)
            ...
            logger.warning("Setup script not found at %s. Skipping auto-setup.", s_path)
