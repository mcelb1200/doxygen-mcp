...
async def query_project_reference(
    ...
    try:
        engine, _ = await _get_engine(project_path)
...
async def get_project_structure(project_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    try:
        engine, resolved_path = await _get_engine(project_path)
...
async def refresh_index(project_path: Optional[str] = None) -> str:
    ...
    """
    try:
        # Using force_refresh=True to clear cache before creating engine
        await _get_engine(project_path, force_refresh=True)
