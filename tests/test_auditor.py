...
def test_find_nm_tool():
    ...
    with patch.dict(os.environ, {}, clear=True):
        with patch("shutil.which", side_effect=lambda x: f"/path/to/{x}" if x == "nm" else None):
...
def test_find_build_dir():
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Test env override
        with patch.dict(os.environ, {"DOXYGEN_BUILD_DIR": str(temp_path / "custom_build")}):
...
def test_scan_binary_gaps(mock_run):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        mock_process.stdout = "         U undefined_symbol\n         U __ignored_symbol\n"
