...
from doxygen_mcp.server import doxy_references, doxy_rename_impact, doxy_parity_check, doxy_refresh_delta
...
def temp_xml_dir():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Write compound XML
        (xml_dir / "class_calculator.xml").write_text(CALCULATOR_XML_CONTENT, encoding="utf-8")
        
        # Write source code file for references
        src_content = "\n" * 14 + "double Calculator::subtract(double a, double b) {\n    return add(a, -b);\n}\n"
...
async def test_doxy_parity_check_tool(mock_xml_dir, mock_resolve_path, temp_xml_dir):
    ...
    assert len(res) > 0
    assert any(m["kind"] in ("parameter_mismatch", "parameter_redundant", "parameter_missing") for m in res)
...
async def test_doxy_refresh_delta_tool(mock_exec, mock_doxy_exe, mock_xml_dir, mock_resolve_path, temp_xml_dir):
    mock_resolve_path.return_value = Path("/tmp/fake_project")
    mock_xml_dir.return_value = temp_xml_dir
    ...
    with tempfile.TemporaryDirectory() as fake_project:
        ...
        async def mock_communicate():
            ...
            (temp_xml_out / "class_calculator.xml").write_text(CALCULATOR_XML_CONTENT, encoding="utf-8")
