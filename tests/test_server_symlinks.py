...
def test_create_project_follow_symlinks():
    ...
    async def _test():
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert "[SUCCESS] Doxygen project 'Test Symlinks' created successfully" in (result.message or result.error or "")
...
def test_create_project_default_no_symlinks():
    ...
    async def _test():
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert result.success is True
            assert "[SUCCESS] Doxygen project 'Test Secure' created successfully" in (result.message or result.error or "")
