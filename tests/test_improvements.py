...
async def test_generate_context_report(mock_engine_create, mock_git_diff, mock_lang, mock_xml_dir, mock_resolve_path):
...
async def test_generate_architecture_review(mock_popen, mock_engine_create, mock_xml_dir, mock_resolve_path):
    mock_resolve_path.return_value = Path("/tmp/fake_project")
    mock_xml_dir.return_value = "/tmp/fake_project/docs/xml"
    ...
    assert "✅ Architecture review generated at" in result or "architecture review" in result.lower()
