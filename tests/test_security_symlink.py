...
def test_symlink_protection_gitignore(tmp_path):
    ...
    assert (
        target_file.read_text(encoding="utf-8") == "sensitive data"
...
def test_symlink_protection_doxyfile(tmp_path):
    ...
    async def run_test():
        ...
        # Mock dependencies
        with patch(
            "doxygen_mcp.server.resolve_project_path", return_value=project_root
        ):
            with patch("doxygen_mcp.server.update_ignore_file", return_value=True):
                with patch(
                    "doxygen_mcp.server.detect_primary_language", return_value="python"
                ):
                    result = await doxy_create("TestProject", str(project_root))

        assert (
            "Security Error" in result
        ), f"Should return security error, got: {result}"
        assert (
            target_file.read_text(encoding="utf-8") == "sensitive data"
        ), "Should not modify target file"
...
def test_overwrite_protection_doxyfile(tmp_path):
    ...
    async def run_test():
        ...
        assert (
            "Doxyfile already exists" in result
        ), f"Should return exists error, got: {result}"
        assert (
            doxyfile.read_text(encoding="utf-8") == "existing content"
