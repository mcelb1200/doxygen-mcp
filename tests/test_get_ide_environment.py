...
def test_get_ide_environment_unknown():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with (
            patch("doxygen_mcp.utils.resolve_project_path", return_value=temp_path),
...
def test_get_ide_environment_vscode_ipc():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with (
            ...
            patch("doxygen_mcp.utils.get_project_name", return_value="test_project"),
            patch.dict(
                os.environ, {"VSCODE_GIT_IPC_HANDLE": "some_handle"}, clear=True
            ),
        ):
...
def test_get_ide_environment_cursor_ipc():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        temp_path = Path(temp_dir)
        env = {"TERM_PROGRAM": "vscode", "CURSOR_GIT_IPC_HANDLE": "some_handle"}
        with (
            patch("doxygen_mcp.utils.resolve_project_path", return_value=temp_path),
            patch("doxygen_mcp.utils.get_project_name", return_value="test_project"),
